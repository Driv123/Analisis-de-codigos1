...
// Funcion para simular la carga de una pagina a memoria fisica usando LRU
void loadPage(FrameList *frameList, int page) {
    ...
    // Si la lista de frames ya esta llena, elimina el frame menos recientemente usado (LRU)
    if (frameList->numFrames == NUM_FRAMES) {
        Frame *lruFrame = frameList->tail;  // El ultimo frame es el menos recientemente usado
        removeFrame(frameList, lruFrame);   // Elimina el frame menos recientemente usado
    }
